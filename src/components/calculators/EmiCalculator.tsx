...
type EmiFormValues = z.infer<typeof emiCalculatorSchema>;
...
export default function EmiCalculator() {
  ...
  useEffect(() => {
    ...
    const subscription = form.watch(() => calculateEmi(form.getValues()));
  ...
  const handleExport = () => {
    ...
    exportToCsv(`emi-schedule.csv`, result.amortization);
            ...
            {result && (
                <>
                    <div className="space-y-1 text-center">
                        ...
                        <p className="font-headline text-4xl font-bold text-primary">{new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(result.monthlyEmi)}</p>
                    </div>
                    <div className="flex justify-around pt-4 text-center">
                        ...
                        <div>
                            ...
                            <p className="text-lg font-semibold">{new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(result.totalPayment)}</p>
                        </div>
                    </div>
