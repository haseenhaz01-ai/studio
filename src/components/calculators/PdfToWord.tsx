...
type PdfToWordFormValues = z.infer<typeof splitPdfSchema>;

export default function PdfToWord() {
  const [fileName, setFileName] = useState<string | null>(null);
  const { toast } = useToast();

  const form = useForm<PdfToWordFormValues>({
  ...
  const handleConvert = () => {
    toast({
        title: 'Coming Soon!',
        description: 'PDF to Word conversion is under construction.'
    });
  }
  ...
  return (
    <Card className="mx-auto max-w-2xl">
      <CardHeader>
        <CardTitle className="font-headline">PDF to Word Converter</CardTitle>
        <CardDescription>
          Convert your PDF to an editable Word document. (Feature coming soon)
        </CardDescription>
      </CardHeader>
