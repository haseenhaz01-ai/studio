...
type PdfToExcelFormValues = z.infer<typeof splitPdfSchema>;

export default function PdfToExcel() {
  const [fileName, setFileName] = useState<string | null>(null);
  const { toast } = useToast();

  const form = useForm<PdfToExcelFormValues>({
  ...
  const handleConvert = () => {
    toast({
        title: 'Coming Soon!',
        description: 'PDF to Excel conversion is under construction.'
    });
  }
  ...
  return (
    <Card className="mx-auto max-w-2xl">
      <CardHeader>
        <CardTitle className="font-headline">PDF to Excel Converter</CardTitle>
        <CardDescription>
          Extract data and tables from your PDF into an Excel spreadsheet. (Feature coming soon)
        </CardDescription>
      </CardHeader>
