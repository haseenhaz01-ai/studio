...
type PdfToJpgFormValues = z.infer<typeof splitPdfSchema>;

export default function PdfToJpg() {
  const [fileName, setFileName] = useState<string | null>(null);
  const { toast } = useToast();

  const form = useForm<PdfToJpgFormValues>({
  ...
  const handleConvert = () => {
    toast({
        title: 'Coming Soon!',
        description: 'PDF to JPG conversion is under construction.'
    });
  }
  ...
  return (
    <Card className="mx-auto max-w-2xl">
      <CardHeader>
        ...
        <CardDescription>
          Convert each page of a PDF into a JPG image. (Feature coming soon)
        </CardDescription>
      </CardHeader>
